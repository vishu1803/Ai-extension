...
import { Copy, Download, ExternalLink, MessageSquarePlus, X } from 'lucide-preact';
...
export function TransferDialog({ onClose }: TransferDialogProps) {
  ...
  }, [currentSummary, format, target, includeMemory, includeCompleted, includePending, nextPrompt, editMode]);
  ...
  const handleDownload = () => {
    const result = TransferEngine.generateExport(currentSummary, {
      format, target, includeMemory, includeCompleted, includePending, nextPrompt
    });
      ...
      <div className="bg-[var(--bg-primary)] border border-[var(--border-subtle)] rounded-xl w-full max-w-3xl max-h-[90vh] flex flex-col shadow-2xl animate-in zoom-in-95 duration-200">
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="w-64 border-r border-[var(--border-subtle)] p-5 overflow-y-auto flex flex-col gap-6 bg-[var(--bg-secondary)]">
            ...
            <div>
              <label className="text-xs font-semibold uppercase tracking-wider text-[var(--text-secondary)] mb-3 block">Target Platform</label>
              <select 
                value={target} 
            ...
            <div>
              <label className="text-xs font-semibold uppercase tracking-wider text-[var(--text-secondary)] mb-3 block">Export Format</label>
            ...
            <div>
              <label className="text-xs font-semibold uppercase tracking-wider text-[var(--text-secondary)] mb-3 block">Include Content</label>
              <div className="flex flex-col gap-3">
                <label className="flex items-center gap-2 text-sm cursor-pointer">
                  <input type="checkbox" checked={includeMemory} onChange={e => setIncludeMemory((e.target as HTMLInputElement).checked)} className="rounded text-[var(--accent-cyan)]" />
                  Project Memory & Goals
                </label>
                <label className="flex items-center gap-2 text-sm cursor-pointer">
                  <input type="checkbox" checked={includeCompleted} onChange={e => setIncludeCompleted((e.target as HTMLInputElement).checked)} className="rounded text-[var(--accent-cyan)]" />
                  Completed Work & Files
                </label>
                <label className="flex items-center gap-2 text-sm cursor-pointer">
                  <input type="checkbox" checked={includePending} onChange={e => setIncludePending((e.target as HTMLInputElement).checked)} className="rounded text-[var(--accent-cyan)]" />
                  Pending Tasks & Bugs
                </label>
              </div>
            </div>
          ...
          <div className="flex-1 flex flex-col p-5 bg-[var(--bg-primary)] relative">
            <div className="flex justify-between items-center mb-2">
              <label className="text-xs font-semibold uppercase tracking-wider text-[var(--text-secondary)]">Live Preview</label>
        ...
        <div className="p-4 border-t border-[var(--border-subtle)] flex justify-end gap-3 bg-[var(--bg-secondary)] rounded-b-xl">
          <button onClick={handleDownload} className="flex items-center gap-2 px-4 py-2 text-sm font-medium border border-[var(--border-subtle)] hover:bg-[var(--bg-tertiary)] rounded-lg transition-colors">
            <Download size={16} /> Download
          </button>
          <button onClick={handleCopy} className="flex items-center gap-2 px-4 py-2 text-sm font-medium border border-[var(--border-subtle)] hover:bg-[var(--bg-tertiary)] rounded-lg transition-colors">
            <Copy size={16} /> Copy to Clipboard
          </button>
          <button onClick={handleOpenNewChat} className="flex items-center gap-2 px-5 py-2 text-sm font-medium bg-[var(--accent-gradient)] text-white hover:opacity-90 shadow-lg hover:shadow-xl rounded-lg transition-all">
